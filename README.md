...
`1.2.14` 为正式版，完成统一资产索引、带令牌的资产浏览模块，以及脚本、缓存和 Table 数据加载架构升级。
...
资产模块现支持根据统一索引中的当前游戏版本标记新增或覆盖的图片、Json 文件，并通过“仅看新增”筛选快速定位当前版本内容；根目录提供游戏图标、游戏地图的快捷跳转，桌面端和移动端目录均可使用。
